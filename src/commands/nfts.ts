...
//Propose the upgrade of a remote contract
  export async function mintTestNFT(): Promise<void> {
    network.setCurrent("testnet");
  
    const sc = getSmartContract("nativo.dokxo.testnet","alantest.testnet");
    const extra_data= "{'culture':'Nuevo NFT ','country':'Nuevo NFT ','creator':'alantest.testnet','price':'1000000000000000000000000','on_sale':true,'on_auction':false,'adressbidder':'accountbidder','highestbidder':'notienealtos','lowestbidder':'notienebajos','expires_at':'NaN','starts_at':'1641169132368.648'}"
  
    const callMethod = await sc.call("minar", {
      token_owner_id: "alantest.testnet",
      token_metadata: {
        title: "Nuevo NFT",
        description: "Nuevo NFT Nuevo NFT Nuevo NFT Nuevo NFT Nuevo NFT Nuevo NFT",
        media: "bafybeiazxwkribnsxjtdftgxhn62r7aidv7qo3xxyvqovjyd7caihkzpd4",
        media_hash: "hashhashhashhashhashhashhashhash",
        extra: extra_data
     }
    }
    , 200, "100000000000000000000000".toString());
  
    console.log(inspect(callMethod, false, 5, true));
